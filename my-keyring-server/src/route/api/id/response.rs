...
pub async fn post_ulid(
        ...
        match request_client_id {
            Some(sse) => {
                if let Some((pr, k)) = get_sse_data(&sse.data) {
                    ...
                    return Ok(new_responder(timing).status(StatusCode::CONFLICT));
                }
            }
            None => {
                warn!("SSE stream id does not exists");
                return Ok(new_responder(timing).status(StatusCode::NOT_FOUND));
            }
        }
    };
    ...
    let response_id = {
        ...
        match response_id {
            ...
            Err(e) => {
                warn!("{:?}", e);
                return Ok(new_responder(timing).status(StatusCode::FORBIDDEN));
            }
        }
    };
    ...
    Ok(new_responder(timing).status(StatusCode::OK).body(format!(
        "id: {}\t{}\t{:?}",
        id,
        Ulid::new(),
        sent
